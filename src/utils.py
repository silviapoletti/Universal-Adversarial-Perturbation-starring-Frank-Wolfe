...
def get_image_perturbated(perturbation, image_test, title, axis):
...
def plot_perturbated_img(perturbation, image_test, file_path=None, figsize=(5,5)):
    """
    ...
    ax[1].set_title("Perturbated image")
...
def evaluate_perturbated_images(lenet5, images, labels, delta, verbose=0):
    deltas = np.tile(delta, images.shape[0])
    images_noise = images + deltas.reshape(images.shape[0], 28, 28, 1)
    images_noise = np.clip(images_noise, 0., 1.)
